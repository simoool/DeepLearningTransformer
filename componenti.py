...
class MultiHeadAttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value, mask):
        ...
        query_output = query_output.view(dim, -1, self.n_heads, self.head_size).permute(0, 2, 1, 3)
        key_output = key_output.view(dim, -1, self.n_heads, self.head_size).permute(0, 2, 1, 3)
        value_output = value_output.view(dim, -1, self.n_heads, self.head_size).permute(0, 2, 1, 3)
...
class Transformer(nn.Module):
    ...
    def forward(self, input, target):
        ...
        input_mask = self.crea_input_mask(input)
        target_mask = self.crea_target_mask(target)
